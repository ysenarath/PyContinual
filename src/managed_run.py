...
if args.backbone == "w2v" or args.backbone == "w2v_as":
    data, taskcla, vocab_size, embeddings = import_modules.dataloader.get(
...
if args.multi_gpu and args.distributed:
    ...
    torch.distributed.init_process_group("nccl", init_method="env://", world_size=n_gpu)
    net = net.to(device)
...
for t, ncla in taskcla:
    ...
    if args.task == "asc":  # special setting
        if "XuSemEval" in data[t]["name"]:
            ...
            num_train_steps *= (
                args.bingdomains_num_train_epochs_multiplier
            )  # every task got refresh, *3

    if args.multi_gpu and args.distributed:
        valid_sampler = DistributedSampler(valid)  # TODO: DitributedSequentailSampler
        valid_dataloader = DataLoader(
            valid, sampler=valid_sampler, batch_size=args.eval_batch_size
        )
    else:
    ...
    if args.resume_model and t < args.resume_from_task:
        continue  # resume. dont forget to copy the forward results

    if args.multi_gpu and args.distributed:
        train_sampler = DistributedSampler(train)
        train_dataloader = DataLoader(
            train, sampler=train_sampler, batch_size=args.train_batch_size
        )
    else:
    ...
    if not args.eval_only:
        if args.task in extraction_tasks:
            ...
            print("train")
            appr.train(
                task, train_dataloader, valid_dataloader, num_train_steps, train, valid
            )

    print("-" * 100)

    if (
        args.exit_after_first_task
    ):  # sometimes we want to fast debug or estimate the excution time
        # TODO: consider save to file and print them out
        logger.info(
            "[Elapsed time per epochs = {:.1f} s]".format((time.time() - tstart))
        )
        logger.info(
            "[Elapsed time per epochs = {:.1f} min]".format(
                (time.time() - tstart) / (60)
            )
        )
        logger.info(
            "[Elapsed time per epochs = {:.1f} h]".format(
                (time.time() - tstart) / (60 * 60)
            )
        )
        if (
            "kim" not in args.baseline
            and "mlp" not in args.baseline
            and "cnn" not in args.baseline
        ):
            if "asc" in args.task:
            ...
            logger.info(
                "[Elapsed time per tasks = {:.1f} s]".format(
                    (time.time() - tstart) * pre_define_num_epochs
                )
            )
            logger.info(
                "[Elapsed time per tasks = {:.1f} min]".format(
                    ((time.time() - tstart) / (60)) * pre_define_num_epochs
                )
            )
            logger.info(
                "[Elapsed time per tasks = {:.1f} h]".format(
            ...
            logger.info(
                "[Elapsed time per tasks = {:.1f} s]".format(
                    (time.time() - tstart) * 50 * additional
                )
            )  # estimation for early stopping
            logger.info(
                "[Elapsed time per tasks = {:.1f} min]".format(
                    ((time.time() - tstart) / (60)) * 50 * additional
                )
            )
            logger.info(
                "[Elapsed time per tasks = {:.1f} h]".format(
                    ((time.time() - tstart) / (60 * 60)) * 50 * additional
                )
            )
        exit()
    ...
    if args.save_model:
        ...
        if hasattr(appr, "grad_dims"):
            torch.save(
                appr.grad_dims, args.model_path + "_grad_dims"
            )  # not in state_dict
        if hasattr(appr, "grads_cs"):
            torch.save(
                appr.grads_cs, args.model_path + "_grads_cs"
            )  # not in state_dict
        if hasattr(appr, "grads_da"):
            torch.save(
                appr.grads_da, args.model_path + "_grads_da"
            )  # not in state_dict
        if hasattr(appr, "history_mask_pre"):
            torch.save(
                appr.history_mask_pre, args.model_path + "_history_mask_pre"
            )  # not in state_dict
        if hasattr(appr, "similarities"):
            torch.save(
                appr.similarities, args.model_path + "_similarities"
            )  # not in state_dict
        if hasattr(appr, "check_federated"):
            torch.save(
                appr.check_federated, args.model_path + "_check_federated"
            )  # not in state_dict
        ...
        if args.aux_net:
            ...
            if hasattr(appr, "mask_pre"):
                torch.save(
                    appr.mask_pre, args.aux_model_path + "_mask_pre"
                )  # not in state_dict
            ...
            if hasattr(appr, "mask_pre"):
                torch.save(
                    appr.mask_pre, args.model_path + "_mask_pre"
                )  # not in state_dict
            if hasattr(appr, "mask_back"):
    ...
    for u in range(test_set):
        ...
        if args.multi_gpu and args.distributed:
            ...
            test_dataloader = DataLoader(
                test, sampler=test_sampler, batch_size=args.eval_batch_size
            )
        else:
        ...
        if args.task in classification_tasks:  # classification task
            if "kan" in args.baseline:
                test_loss, test_acc, test_f1_macro = appr.eval(
                    u, test_dataloader, test, which_type="mcl", trained_task=t
                )
                logger.info(
                ...
                test_loss, test_acc, test_f1_macro = appr.eval(
                    u, test_dataloader, valid_dataloader, trained_task=t, phase="mcl"
                )
                logger.info(
                    ">>> Test on task {:2d} - {:15s}: loss={:.3f}, acc={:5.1f}% <<<".format(
                        u, data[u]["name"], test_loss, 100 * test_acc
                ...
                test_loss, test_acc, test_f1_macro = appr.eval(
                    u, test_dataloader, test, trained_task=t
                )
                logger.info(
                    ">>> Test on task {:2d} - {:15s}: loss={:.3f}, acc={:5.1f}% <<<".format(
                        u, data[u]["name"], test_loss, 100 * test_acc
            ...
            np.savetxt(
                args.output + "progressive.f1_macro", f1_macro, "%.4f", delimiter="\t"
            )
            ...
            print(
                "[Elapsed time = {:.1f} h]".format((time.time() - tstart) / (60 * 60))
            )

            with open(performance_output, "w") as file, open(
                f1_macro_output, "w"
            ) as f1_file:
            ...
            with open(performance_output_forward, "w") as file, open(
                f1_macro_output_forward, "w"
            ) as f1_file:
